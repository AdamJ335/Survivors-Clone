...
func _ready():
	...
	match last_movement:
		Vector2.UP, Vector2.DOWN:
			# get tornado spawn, add 500 pixels to random direction to move
		...
		Vector2(1,1), Vector2(-1,-1), Vector2(1,-1), Vector2(-1,1): #top_left, bottom_right, bottom_left, top_right 
	...
	var other_angle = Vector2.ZERO
	if set_angle == 1:
		angle = angle_less
		other_angle = angle_more
	else:
		angle = angle_more
		other_angle = angle_less
	for i in 8: # tween animation
		tween.tween_property(self, "angle", angle, 2)
		tween.tween_property(self, "angle", other_angle, 2)
	tween.play()
