...
func _physics_process(delta: float) -> void:
	...
	else: # No targets, float back to player
...
func add_paths():
	...
	target_array.clear() #clears the targets to hit
	var counter = 0
	while counter < paths: # add new targets for every time a javelin can move per attack
		var new_path = player.get_random_target()
		target_array.append(new_path)
		counter += 1
	enable_attack(true) # enter attack mode on sprite
...
func _on_change_direction_timeout() -> void:
	if target_array.size() > 0: # have we timed out without hitting target?
		target_array.remove_at(0) # remove target
		if target_array.size() > 0: # do we still have a target to hit?
			target = target_array[0] # focus on new target & attack
			process_path()
			snd_attack.play()
			...
			attackTimer.start()
			enable_attack(false)
	else: #code no longer run, but kept as failsafe 
		changeDirectionTimer.stop()
		attackTimer.start()
		enable_attack(false)
